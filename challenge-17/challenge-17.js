...
var newText = text.replace(/Manuel Marques de Sousa/g, 'Mateus da Silva Sousa')
console.log(newText)
...
newText = text.replace(/brasileiro/, 'mineiro')
console.log(newText);
...
newText = text.replace(/(O Centauro de Luvas)/g, function($1){
...
console.log(`O mês de ${dateMonth} é representado pelo número ${getMonthNumber(dateMonth)}.`)
...
var regexDate = /(\d{2}) de (junho|julho) de (\d{4})/g
...
console o resultado.
*/
function replaceDate(total, $1, $2, $3){
    $2 = getMonthNumber($2)
    return `${$1}/${$2}/${$3}`
}
console.log( '\nReplace de datas:' );
newText = text.replace(regexDate, replaceDate)
console.log(newText)
})()